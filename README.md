...
│       ├── analyzers.py             # Pipeline validazione testo (NO parsing)
...
### ✅ Pipeline Analizzatore (Opzionale, NO Parsing)
- Post-elaborazione dei documenti
- Solo validazioni non strutturali (es. controllo lunghezza testo, rimozione null-byte)
- Nessun parsing/strutturazione HTML, XML o JSON in analyzer
...
### Completo (Tutti i formati + metadati, senza parsing analyzer)
```yaml
ingest:
  input:
